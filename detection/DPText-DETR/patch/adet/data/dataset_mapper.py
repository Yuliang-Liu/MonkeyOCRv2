...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __init__(self, cfg, is_train=True):
        ...
        if cfg.INPUT.CROP.ENABLED and is_train and not cfg.INPUT.FIXED_SIZE.ENABLED:
        ...
        elif cfg.INPUT.CROP.ENABLED and is_train and cfg.INPUT.FIXED_SIZE.ENABLED:
            logging.getLogger(__name__).info(
                "Skip mapper pre-crop because INPUT.FIXED_SIZE.ENABLED=True; crop is handled in fixed-size augmentation."
            )
    ...
    def __call__(self, dataset_dict):
        ...
        valid_image_shape = self._infer_valid_image_shape(transforms, image_shape)
        ...
        dataset_dict["valid_size"] = valid_image_shape
        ...
        if "annotations" in dataset_dict:
            ...
            instances = annotations_to_instances(
                annos, valid_image_shape, mask_format=self.instance_mask_format
            )
    ...
    @staticmethod
    def _infer_valid_image_shape(transforms, final_image_shape):
        flat_transforms = []
        ...
        _flatten(transforms)
        for transform in reversed(flat_transforms):
            if isinstance(transform, T.PadTransform):
                return int(transform.orig_h), int(transform.orig_w)
        return int(final_image_shape[0]), int(final_image_shape[1])
