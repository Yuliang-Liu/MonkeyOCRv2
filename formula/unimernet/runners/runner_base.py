...
class RunnerBase:
    ...
    def optimizer(self):
        ...
        if self._optimizer is None:
            ...
            def component_and_scale(name):
                name = name[7:] if name.startswith("module.") else name
                # Support both wrapped UniMERModel names and direct
                # DonutEncoderDecoder names so component LR scaling is applied.
                if name.startswith("model.model."):
                    name = "model." + name[len("model.model."):]
                if name.startswith("model.encoder_bridge."):
                    return "encoder_bridge", float(self.config.run_cfg.get("encoder_bridge_lr_scale", 1.0))
                if name.startswith("model.encoder."):
                    return "encoder", float(self.config.run_cfg.get("encoder_lr_scale", 1.0))
                if name.startswith("model.enc_to_dec_proj."):
                    return "enc_to_dec_proj", float(self.config.run_cfg.get("enc_to_dec_proj_lr_scale", 1.0))
                if name.startswith("model.decoder."):
                    return "decoder", float(self.config.run_cfg.get("decoder_lr_scale", 1.0))
                return "other", float(self.config.run_cfg.get("other_lr_scale", 1.0))

            for n, p in self.model.named_parameters():
                if not p.requires_grad:
                    continue  # frozen weights
                component, lr_scale = component_and_scale(n)
                ...
                key = (component, decay_name, lr_scale)
                if key not in groups:
                    groups[key] = {
                        ...
                        "lr_scale": lr_scale,
                        "lr": init_lr * lr_scale,
                        "name": f"{component}_{decay_name}_lrscale_{lr_scale:g}",
                    }
                groups[key]["params"].append(p)
                num_parameters += p.data.nelement()
            logging.info("number of trainable parameters: %d" % num_parameters)
            optim_params = list(groups.values())
            for group in optim_params:
                logging.info(
                    "optimizer group %s: tensors=%d, lr_scale=%s, weight_decay=%s",
                    group["name"],
                    len(group["params"]),
                    group["lr_scale"],
                    group["weight_decay"],
    ...
    def create_loaders(
        ...
        def _create_loader(dataset, num_workers, bsz, is_train, collate_fn):
            ...
            if isinstance(dataset, ChainDataset) or isinstance(
                ...
                use_token_bucket = (
                    is_train
                    and bool(self.config.run_cfg.get("token_bucket_batching", False))
                    and hasattr(dataset, "estimate_vision_tokens")
                )
                if use_token_bucket:
                    lengths = [dataset.estimate_vision_tokens(i) for i in range(len(dataset))]
                    if all(length is not None for length in lengths):
                        batch_sampler = TokenBucketBatchSampler(
                            ...
                            shuffle=True,
                            drop_last=True,
                            bucket_size=int(self.config.run_cfg.get("token_bucket_size", 2048)),
                            seed=int(self.config.run_cfg.get("seed", 0)),
                            num_replicas=get_world_size() if self.use_distributed else 1,
                            rank=get_rank() if self.use_distributed else 0,
                        )
                        logging.info(
                            "Using token bucket batch sampler: batch_size=%s, bucket_size=%s, batches_per_rank=%s",
                            bsz,
                            self.config.run_cfg.get("token_bucket_size", 2048),
                            len(batch_sampler),
                        )
                        loader = DataLoader(
                            dataset,
                        ...
                        logging.warning("token_bucket_batching requested but token estimates are unavailable; using default loader.")
                        loader = DataLoader(
                            dataset,
                            batch_size=bsz,
                            num_workers=num_workers,
