...
from transformers import VisionEncoderDecoderModel, AutoImageProcessor, MBartForCausalLM
...
from .modeling_monkeyocrv2_encoder import MonkeyOCRv2VisionTransformer
from .modeling_unimernet_decoder import MBartDecoder
from .adapter_resampler import MonkeyAdapterResamplerBridge
...
class CustomVisionEncoderDecoderModel(VisionEncoderDecoderModel):
    def __init__(self, config):
        ...
        if isinstance(config.encoder, MonkeyOCRv2VisionConfig):
            encoder = MonkeyOCRv2VisionTransformer(config.encoder)
        else:
            encoder = VariableUnimerNetModel(config.encoder)
        decoder = CustomMBartForCausalLM(config.decoder)
        super().__init__(config=config, encoder=encoder, decoder=decoder)
        self.encoder = encoder
        self.decoder = decoder
        self.encoder_bridge = None
        if isinstance(config.encoder, MonkeyOCRv2VisionConfig) and config.encoder.use_2d_token_bridge:
            bridge_output_dim = config.encoder.bridge_output_dim or config.decoder.d_model
            if bridge_output_dim != config.decoder.d_model:
                raise ValueError(
                    f"Bridge output dim {bridge_output_dim} must match decoder d_model {config.decoder.d_model}."
                )
            if config.encoder.bridge_target_grid is None:
                raise ValueError("bridge_target_grid must be provided when use_2d_token_bridge=True.")
            self.encoder_bridge = MonkeyAdapterResamplerBridge(
                input_dim=config.encoder.embed_dim,
                output_dim=bridge_output_dim,
                merge_size=config.encoder.spatial_merge_size,
                target_grid=tuple(config.encoder.bridge_target_grid),
                num_local_blocks=config.encoder.bridge_num_local_blocks,
                init_std=config.encoder.init_merger_std,
            )
            if hasattr(self, "enc_to_dec_proj") and self.enc_to_dec_proj is not None:
                # The 2D bridge already maps encoder tokens to decoder width, so the
                # inherited 384->512 projection becomes unreachable and breaks DDP.
                for param in self.enc_to_dec_proj.parameters():
                    param.requires_grad = False

    def _resolve_bridge_grid_size(
        self,
        encoder_outputs: BaseModelOutput,
        pixel_values: Optional[torch.FloatTensor],
    ) -> Tuple[int, int]:
        grid_size = getattr(encoder_outputs, "grid_size", None)
        if grid_size is not None:
            return tuple(grid_size)
        if pixel_values is None:
            raise ValueError("Bridge requires encoder grid_size when pixel_values is unavailable.")
        ...
        **kwargs,
        ...
        if encoder_outputs is None:
            ...
            if image_grid_thw is not None:
                kwargs_encoder["image_grid_thw"] = image_grid_thw
        ...
        if self.encoder_bridge is not None:
            cached_bridge = encoder_outputs.get("bridged_last_hidden_state", None)
            if cached_bridge is None:
                encoder_hidden_states = self.encoder_bridge(
                    encoder_hidden_states,
                    grid_size=self._resolve_bridge_grid_size(encoder_outputs, pixel_values),
                    token_order=getattr(
                        encoder_outputs,
                        "token_order",
                        getattr(self.encoder, "token_order", "row_major"),
                    ),
                )
                encoder_outputs["bridged_last_hidden_state"] = encoder_hidden_states
            else:
                encoder_hidden_states = cached_bridge
            bridge_applied = True
...
class DonutEncoderDecoder(nn.Module):
    ...
    def __init__(
        ...
        bos_token_id,
        eos_token_id,
        encoder_backend="unimernet",
        monkey_encoder_path=None,
        monkey_use_native_token_order=False,
        monkey_use_adapter_bridge=False,
        monkey_bridge_output_dim=None,
        monkey_bridge_target_grid=None,
        monkey_bridge_num_local_blocks=1,
    ):
        super().__init__()
        config = VisionEncoderDecoderConfig.from_pretrained(model_name)
        if encoder_backend == "monkeyocrv2":
            if monkey_encoder_path is None:
                raise ValueError("monkey_encoder_path must be provided when encoder_backend='monkeyocrv2'.")
            encoder = MonkeyOCRv2VisionConfig.from_pretrained(monkey_encoder_path)
            encoder.use_native_token_order = monkey_use_native_token_order
            encoder.use_2d_token_bridge = monkey_use_adapter_bridge
            encoder.bridge_output_dim = monkey_bridge_output_dim
            encoder.bridge_target_grid = list(monkey_bridge_target_grid) if monkey_bridge_target_grid is not None else None
            encoder.bridge_num_local_blocks = monkey_bridge_num_local_blocks
            encoder.hidden_size = encoder.embed_dim
        else:
        ...
        if encoder_backend == "monkeyocrv2":
            self.model.encoder.load_weights(monkey_encoder_path)
    ...
    def forward(self, pixel_values, decoder_input_ids, decoder_attention_mask, image_grid_thw=None, **kwargs):
        if isinstance(pixel_values, dict):
            image_grid_thw = pixel_values.get("image_grid_thw", image_grid_thw)
            pixel_values = pixel_values["pixel_values"]

        num_channels = pixel_values.shape[1] if pixel_values.ndim == 4 else None
        if num_channels == 1:
    ...
    def generate(self, pixel_values, temperature, max_new_tokens, decoder_start_token_id, do_sample, top_p,
        ...
        encoder_kwargs = {}
        if image_grid_thw is not None:
            encoder_kwargs["image_grid_thw"] = image_grid_thw
        encoder_outputs = self.model.encoder(
            pixel_values,
            return_dict=True,
            **encoder_kwargs,
        )
        if image_grid_thw is not None and pixel_values.ndim == 2:
            batch_size = int(image_grid_thw[:, 0].sum().item())
            generate_shape = (
                batch_size,
                self.model.encoder.config.num_channels,
                self.model.encoder.config.patch_size,
                self.model.encoder.config.patch_size,
            )
        else:
        ...
        generate_pixel_values = pixel_values.new_zeros(generate_shape)
