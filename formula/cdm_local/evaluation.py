...
def deterministic_ransac(data, model_class, min_samples, residual_threshold, max_trials):
    kwargs = dict(
        min_samples=min_samples,
        residual_threshold=residual_threshold,
        max_trials=max_trials,
    )
    ransac_params = inspect.signature(ransac).parameters
    if "random_state" in ransac_params:
        kwargs["random_state"] = 42
    else:
        kwargs["rng"] = 42
    return ransac(data, model_class, **kwargs)
...
def evaluation(data_root, user_id="test"):
    ...
    save_match_vis = os.environ.get("CDM_SAVE_MATCH_VIS", "1") != "0"
    if save_match_vis:
        ...
        if src.shape[0] <= min_samples:
            ...
            for i in range(max_iter):
                if src[inliers==False].shape[0] <= min_samples:
                    break
                model, inliers_1 = deterministic_ransac(
    ...
    mean_score = round(float(np.mean(score_list)), 3) if score_list else None
    exp_rate = round(float(np.mean(exp_list)), 3) if exp_list else None
    metrics_res = {
...
if __name__ == '__main__':
    ...
    for subset in ['gt', 'pred']:
        # On NFS, multiprocessing may leave transient .nfs* handles here.
        # This directory is only temporary render scratch space, so cleanup
        # failures should not abort metric calculation.
        shutil.rmtree(os.path.join(temp_dir, f"{exp_name}_{subset}"), ignore_errors=True)
