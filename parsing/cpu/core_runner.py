...
def _ensure_vllm_available():
    global SamplingParams, AsyncLLMEngine, AsyncEngineArgs, _VLLM_IMPORT_ERROR, _VLLM_IMPORTED
    if _VLLM_IMPORTED:
        return
    try:
        ...
        try:
            ...
            from vllm import AsyncLLMEngine as _AsyncLLMEngine, AsyncEngineArgs as _AsyncEngineArgs
        ...
        raise ImportError(
            "vLLM backend is unavailable. Install vLLM/CUDA dependencies or run with "
...
def _resolve_torch_dtype(dtype: str, device: str):
    dtype = (dtype or "auto").lower()
    if dtype == "auto":
        return torch.float32 if device == "cpu" else "auto"
    mapping = {
    ...
    if device == "cpu" and dtype == "float16":
        warnings.warn("float16 is inefficient or unsupported for many CPU ops; using float32 instead.", RuntimeWarning)
        return torch.float32
    return mapping[dtype]


def _install_cpu_sdpa_fallback():
    if getattr(F, "_mocr2_cpu_sdpa_patched", False):
        return

    original_sdpa = F.scaled_dot_product_attention

    def _cpu_sdpa(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=False, scale=None):
        ...
        if dropout_p not in (0.0, 0):
            warnings.warn("CPU SDPA fallback ignores dropout_p.", RuntimeWarning)
        d = query.shape[-1]
    ...
    F.scaled_dot_product_attention = _cpu_sdpa
    F._mocr2_cpu_sdpa_patched = True


class MonkeyOCRv2_TransformersParsing:
    def __init__(
        self,
        model_path: str,
        device: str = "auto",
        ...
        self.dtype = _resolve_torch_dtype(dtype, self.device)
        if self.device == "cpu":
            _install_cpu_sdpa_fallback()
        if (attn_implementation or "auto").lower() == "auto":
        ...
        self.model = AutoModelForCausalLM.from_pretrained(model_path, **model_kwargs).eval().to(self.device)
        self.processor = AutoProcessor.from_pretrained(model_path, trust_remote_code=True)
        if self.device == "cpu" and hasattr(self.model, "vision_tower"):
            vision_tower = self.model.vision_tower
            original_forward = vision_tower.forward

            def _forward_no_bf16(hidden_states, grid_thw, bf16=True):
                return original_forward(hidden_states, grid_thw, bf16=False)

            vision_tower.forward = _forward_no_bf16
    ...
    def _generate_one(
        ...
        inputs = {
            key: (
                value.to(device=self.model.device, dtype=torch.float32)
        ...
        generation_kwargs = {
            "max_new_tokens": int(max_tokens or 10000),
    ...
    def get(self, config: BackendConfig):
        ...
        with self._lock:
            if key not in self._cache:
                ...
                model = MonkeyOCRv2_TransformersParsing(
                    config.model_path,
                    device=device,
                    dtype=dtype,
                    attn_implementation=attn_implementation,
                ...
                print(
                    "Using CPU Transformers backend: "
                    f"model={config.model_path} dtype={dtype} attn={attn_implementation}"
                )
                preprocessor = None
