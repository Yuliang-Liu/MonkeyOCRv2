...
class Seq2SeqTrainer(SwiftMixin, DataLoaderMixin, HfSeq2SeqTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if getattr(outputs, 'aux_loss2', None) is not None:
            mode = 'train' if self.model.training else 'eval'
            self.custom_metrics[mode]['aux_loss2'].update(outputs.aux_loss2)
    ...
    def training_step(self, model, inputs, *args, **kwargs):
        if isinstance(inputs, dict) and '_profile' in inputs and getattr(self, 'state', None) is not None:
            profile_last = getattr(self.state, '_profile_last', {})
            profile_last.update(inputs.pop('_profile'))
        ...
        if getattr(self, 'state', None) is not None:
            profile_last = getattr(self.state, '_profile_last', {})
            profile_last['train_step_s'] = time.perf_counter() - step_start
            self.state._profile_last = profile_last
        return loss
