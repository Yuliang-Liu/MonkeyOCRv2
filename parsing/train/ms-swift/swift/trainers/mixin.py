...
from swift.llm.model.patcher import get_lm_head_model, revert_padding_free, transformers_seq_cls_forward
...
class SwiftMixin:
    ...
    def _install_profile_hooks(self):
        if getattr(self, '_profile_hooks_installed', False):
            return
        self._profile_hooks_installed = True

        original_compute_loss = self.compute_loss

        @wraps(original_compute_loss)
        def _profiled_compute_loss(*args, **kwargs):
            start = time.perf_counter()
            res = original_compute_loss(*args, **kwargs)
            if getattr(self, 'state', None) is not None and self.model.training:
                profile_last = getattr(self.state, '_profile_last', {})
                profile_last['forward_s'] = time.perf_counter() - start
                self.state._profile_last = profile_last
            return res

        self.compute_loss = _profiled_compute_loss
        ...
        def _profiled_backward(loss, *args, **kwargs):
            start = time.perf_counter()
            res = original_backward(loss, *args, **kwargs)
            if getattr(self, 'state', None) is not None:
                profile_last = getattr(self.state, '_profile_last', {})
                profile_last['backward_s'] = time.perf_counter() - start
                self.state._profile_last = profile_last
            return res

        self.accelerator.backward = _profiled_backward

    def _wrap_data_collator_for_profile(self, data_collator):
        if data_collator is None:
            return None

        def _wrapped_data_collator(features):
            start = time.perf_counter()
            lazy_times = []
            for feat in features:
                if isinstance(feat, list):
                    for sub_feat in feat:
                        if isinstance(sub_feat, dict) and '_profile_lazy_tokenize_time' in sub_feat:
                            lazy_times.append(sub_feat.pop('_profile_lazy_tokenize_time'))
                elif isinstance(feat, dict) and '_profile_lazy_tokenize_time' in feat:
                    lazy_times.append(feat.pop('_profile_lazy_tokenize_time'))
            batch = data_collator(features)
            collate_time = time.perf_counter() - start
            if isinstance(batch, dict):
                profile = {}
                if lazy_times:
                    total_lazy = sum(lazy_times)
                    profile['lazy_tokenize_batch_s'] = total_lazy
                    profile['lazy_tokenize_avg_s'] = total_lazy / len(lazy_times)
                profile['data_collator_s'] = collate_time
                batch['_profile'] = profile
            return batch

        return _wrapped_data_collator
    ...
    def __init__(self,
        ...
        data_collator = self._wrap_data_collator_for_profile(data_collator)
    ...
    def log(self, logs: Dict[str, float], *args, **kwargs) -> None:
        ...
        if mode == 'train':
            profile_last = getattr(self.state, '_profile_last', None)
            if profile_last:
                if 'lazy_tokenize_batch_s' in profile_last:
                    logs['time/lazy_tokenize_batch(s)'] = profile_last['lazy_tokenize_batch_s']
                if 'lazy_tokenize_avg_s' in profile_last:
                    logs['time/lazy_tokenize_avg(s)'] = profile_last['lazy_tokenize_avg_s']
                if 'data_collator_s' in profile_last:
                    logs['time/data_collator(s)'] = profile_last['data_collator_s']
                if 'forward_s' in profile_last:
                    logs['time/forward(s)'] = profile_last['forward_s']
                if 'backward_s' in profile_last:
                    logs['time/backward(s)'] = profile_last['backward_s']
                if 'train_step_s' in profile_last:
                    logs['time/train_step(s)'] = profile_last['train_step_s']
                if 'dataloader_wait_s' in profile_last:
                    logs['time/dataloader_wait(s)'] = profile_last['dataloader_wait_s']
...
class DataLoaderMixin:
    ...
    def _wrap_train_dataloader_for_profile(self, dataloader):
        ...
        class _ProfiledIter:
            ...
            def __next__(self):
                ...
                if getattr(trainer, 'state', None) is not None:
                    profile_last = getattr(trainer.state, '_profile_last', {})
                    profile_last['dataloader_wait_s'] = wait
                    trainer.state._profile_last = profile_last
                return batch
        ...
        class _ProfiledDataLoader:
            ...
            def __iter__(self):
                return _ProfiledIter(iter(self._inner))
        ...
        return _ProfiledDataLoader(dataloader)
    ...
    def get_train_dataloader(self, skip_batches=0):
        ...
        return self._wrap_train_dataloader_for_profile(dataloader)
