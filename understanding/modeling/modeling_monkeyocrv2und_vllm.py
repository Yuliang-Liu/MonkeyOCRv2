...
class MonkeyOCRv2ProcessingInfo(Qwen2VLProcessingInfo):
    ...
    def get_num_image_tokens(
        ...
        *,
        ...
        # Qwen2VL's accounting divides patch count by vision_config's 2x2
        # merge size. Understanding's connector is 1x1, so every patch is
        # represented by a language-model token.
        return 4 * super().get_num_image_tokens(
    ...
    def get_image_processor(self, **kwargs: object):
        # Qwen2VLMultiModalProcessor calls this separately from
        # get_hf_processor() when expanding image placeholders. Ensure that
        # path also uses understanding's 1x1 connector semantics.
        processor = super().get_image_processor(**kwargs)
...
class PatchMerger(nn.Module):
    def __init__(
        ...
        super().__init__()
        # The understanding connector does not spatially merge patch features.
        self.spatial_merge_size = spatial_merge_size
    ...
    def __init__(
        ...
        *,
        ...
        self.merger = PatchMerger(
            ...
            # Keep the backbone's spatial ordering (config value) but use a
            # 1x1 connector, as in the native understanding checkpoint.
...
class MonkeyOCRv2ForCausalLM(nn.Module, SupportsMultiModal, SupportsPP, SupportsLoRA, SupportsEagle, SupportsEagle3):
    ...
    def _process_image_input(
        ...
        # Understanding emits one embedding for every patch.
        merge_size = self.vision_tower.merger.spatial_merge_size
        sizes = (
